...
def main():
    ...
    parser.add_argument("--dataset", default="hotpot-all", type=str, choices=['hotpot-all', 'LC-QuAD'],\
                        help="Which dataset use to decompose the questions.")
    ...
    if args.task == "decompose":
        ...
        with open(os.path.join('data', args.dataset, '{}.json'.format(data_type)), 'r') as f:
        ...
        if args.task == 'aggregate-bridge':
            for (d0, d1, d2, context) in zip(data0, data1, data2, contexts):
                ...
                for i in range(30): #args.topk):
                    ...
                    for j in range(30): #args.topk):
                        ...
                        answers.append((answer1, answer2, (i, j), (o1[i]['probability'], o2[j]['probability'])))
                ...
                new_nbest_predictions[d0['id']] = [{
                        ...
                        'q1': d1['question'],
                        'q2': d2['question'].replace('[answer]', answer1),
                        'ans1': answer1,
                        'ans2': answer2,
                        'probability': probability
                    } for (answer1, answer2, (i, j), probability) in answers]
            ...
            for (d0, d1, d2, context) in zip(data0, data1, data2, contexts):
                ...
                o1 = {o['text']:o['probability'] for o in filter_duplicate(out1[d1['id']])}
                o2 = {o['text']:o['probability'] for o in filter_duplicate(out2[d2['id']])}

                o1_items, o2_items = list(o1.items()).copy(), list(o2.items()).copy()
                for (text, probability) in o1_items:
                    tokens = [t for token in text.split('and') for t in token.split(',')]
                    tokens = [t.strip() for t in tokens if len(t.strip())>0]
                    if len(tokens)>0:
                        for token in tokens:
                            o1[token]=probability
                for (text, probability) in o2_items:
                    tokens = [t for token in text.split('and') for t in token.split(',')]
                    tokens = [t.strip() for t in tokens if len(t.strip())>0]
                    if len(tokens)>0:
                        for token in tokens:
                            o2[token]=probability

                combined_answers = [(t, (o1.get(t, 0), o2.get(t, 0))) for t in list(set(o1.keys())|set(o2.keys()))]
                answers_probabilities = sorted(combined_answers, key=lambda x: -sum(x[1]))
                answers = [a[0] for a in answers_probabilities]
                new_pred = []
                for answer, probability in answers_probabilities:
                    bool1 = False
                    bool2 = False
                    if answer in o1:
                        bool1 = True
                    ...
                    if answer in o2:
                        ...
                        evidence2 = ['']
                    ans1 = answer if bool1 else ""
                    ans2 = answer if bool2 else ""
